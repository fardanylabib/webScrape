...
const URL = "https://www.peopleperhour.com/hire-freelancers?location=ID&page=";
...
const scrape = async (selectedStartPage) => {
	...
	const resultDirectory = ".\\Result\\pph_links\\";

	fs.mkdir(resultDirectory,function(e){
	    if(!e || (e && e.code === 'EEXIST')){
	        console.log("directory pph_links created");
	    } else {
	        //debug
	        console.log(e);
	    }
	...
	let failCounter = 0;
  	var links= [];
  	while(failCounter<10 && selectedStartPage< 30){  
...
async function getAllEmployeeLinks(page){
	...
	result = await page.evaluate(()=>{
		let data1 = [];
		let elements = document.querySelectorAll(".freelancer__title h5 .link");
