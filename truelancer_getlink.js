...
const URL = "https://www.truelancer.com/freelancers?clist=ID&page=";
...
const scrape = async (selectedStartPage) => {
	...
	const resultDirectory = ".\\Result\\truelancer_links\\";

	fs.mkdir(resultDirectory,function(e){
	    if(!e || (e && e.code === 'EEXIST')){
	        console.log("directory truelancer_links created");
	    } else {
	        //debug
	        console.log(e);
	    }
	...
	let failCounter = 0;
  	var links= [];
  	while(failCounter<10 && selectedStartPage< 1643){  
...
async function getAllEmployeeLinks(page){
	...
	result = await page.evaluate(()=>{
		let data1 = [];
		let elements = document.querySelectorAll(".visible-xs-block.hidden-sm.hidden-md.hidden-lg.display_box.clearfix.space .col-xs-12 .tcenter.tl_gap_top_5 a")
